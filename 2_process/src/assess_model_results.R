...
summarize_model_results <- function(raw_results_csv) {
  eval_data <- readr::read_csv(raw_results_csv, col_types = 'iccd') %>%
...
write_model_summary_results <- function(eval_data, outfile) {
  readr::write_csv(eval_data, path = outfile)
}

write_results_assessment <- function(eval_data, assessment_txt) {
